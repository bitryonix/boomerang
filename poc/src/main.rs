...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    const MILESTONE_BLOCK_0: u32 = 10;
    const MILESTONE_BLOCK_1: u32 = 200;
    const MILESTONE_BLOCK_2: u32 = 300;
    const MILESTONE_BLOCK_3: u32 = 400;
    const MILESTONE_BLOCK_4: u32 = 500;
    const MILESTONE_BLOCK_5: u32 = 600;

    // Miner settings.
    const INITIAL_MINER_NUM_BLOCKS_TO_MINE: u64 = 150;
    const MINER_TASK_SLEEPING_TIME_IN_MILLISECONDS: u64 = 20;
    const MINER_NUM_BLOCKS_TO_MINE_FOR_DEPOSIT_TRANSACTION_TO_BE_MINED: u64 = 100;

    // Withdrawal transaction settings.
    const DEPOSIT_AMOUNT_TO_BOOMERANG_ADDRESS_IN_INT_BTC: u64 = 21;
    const ABSOLUTE_LOCKTIME_FOR_WITHDRAWAL_TRANSACTION: u32 = 20;
    ...
    const MAX_TRIES_FOR_DIGGING_GAME_IN_BLOCKS: u32 = 25;

    // Block tolerances for withdrawal.
    // Transaction approval.
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_INITIATOR_PEER_TO_TX_APPROVAL_BY_WT: u32 = 5;
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_WT_TO_RECEIVING_WT_TX_APPROVAL_BY_NON_INITIATOR_PEERS: u32 = 10;
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_NON_INITIATOR_PEER_TO_RECEIVING_NON_INITIATOR_PEERS_TX_APPROVAL_BY_WT: u32 = 10;
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_NON_INITIATOR_PEERS_TO_RECEIVING_NON_INITIATOR_TX_APPROVAL_BY_OTHER_NON_INITIATOR_PEERS: u32 = 10;
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_WT_TO_RECEIVING_NON_INITIATOR_TX_APPROVAL_BY_OTHER_NON_INITIATOR_PEERS: u32 = 10;
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_NON_INITIATOR_PEERS_TO_RECEIVING_NON_INITIATOR_TX_APPROVAL_BY_INITIATOR_PEER: u32 = 10;
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_INITIATOR_PEER_TO_RECEIVING_ALL_NON_INITIATOR_TX_APPROVALS_BY_INITIATOR_PEER: u32 = 10;
    const TOLERANCE_IN_BLOCKS_FROM_TX_APPROVAL_BY_WT_TO_RECEIVING_NON_INITIATOR_TX_APPROVAL_BY_NON_INITIATOR_PEERS: u32 =
        50;
    // Transaction commitment.
    const TOLERANCE_IN_BLOCKS_FROM_TX_COMMITMENT_BY_INITIATOR_PEER_TO_RECEIVING_SAR_RESPONSE_BY_WT: u32 = 20;
    const TOLERANCE_IN_BLOCKS_FROM_TX_COMMITMENT_BY_INITIATOR_PEER_TO_RECEIVING_INITIATOR_PEER_TX_COMMITMENT_BY_NON_INITIATOR_PEERS: u32 = 20;
    const TOLERANCE_IN_BLOCKS_FROM_TX_COMMITMENT_BY_NON_INITIATOR_PEER_TO_RECEIVING_NON_INITIATOR_PEERS_TX_COMMITMENT_BY_WT_HAVING_SAR_RESPONSE_BACK_TO_WT: u32 = 10;
    const TOLERANCE_IN_BLOCKS_FROM_TX_COMMITMENT_BY_INITIATOR_AND_NON_INITIATOR_PEERS_TO_RECEIVING_TX_COMMITMENT_BY_ALL_PEERS: u32 = 30;
    // Digging game.
    const TOLERANCE_IN_BLOCKS_FROM_CREATING_PING_TO_RECEIVING_ALL_PINGS_BY_WT_AND_HAVING_SAR_RESPONSE_BACK_TO_WT: u32 = 50;
    const TOLERANCE_IN_BLOCKS_FROM_CREATING_PONG_BY_WT_TO_REVIEWING_THE_PONG_IN_PEERS_BOOMLET: u32 =
        50;
    const TOLERANCE_IN_BLOCKS_FROM_CREATING_PING_BY_OTHER_PEERS_TO_REVIEWING_THE_PING_IN_PEER_BOOMLET: u32 = 50;
    ...
    const REQUIRED_MINIMUM_DISTANCE_IN_BLOCKS_BETWEEN_INITIATOR_PEER_TX_COMMITMENT_AND_RECEIVING_ALL_NON_INITIATOR_TX_COMMITMENT_BY_INITIATOR_PEER: u32 = 0;
    const REQUIRED_MINIMUM_DISTANCE_IN_BLOCKS_BETWEEN_PING_AND_PONG: u32 = 2;
    // Wt sleeping time.
