...
mod tests {
    ...
    fn create_starts_with_blank_setup_state_and_no_identity_state() {
        ...
        assert_eq!(st.state, State::Setup_AfterCreation_BlankSlate);
        assert!(st.boomlet_identity_pubkey.is_none());
        assert!(st.st_identity_privkey.is_none());
        assert!(st.peer_id.is_none());
    }
}
