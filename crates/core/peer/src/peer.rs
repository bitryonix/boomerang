...
mod tests {
    ...
    #[test]
    fn create_starts_with_blank_setup_state_and_no_identity_data() {
        let peer = Peer::create();

        assert_eq!(peer.state, State::Setup_AfterCreation_BlankSlate);
        assert!(peer.network.is_none());
        assert!(peer.peer_id.is_none());
        assert!(peer.peer_tor_address_signed_by_boomlet.is_none());
    }
}
