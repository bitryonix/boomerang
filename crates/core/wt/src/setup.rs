...
impl Wt {
    ...
    /// Initialize WT using pre-generated identity material so external
    /// supervisors can assign a stable WT identity before the process starts.
    #[instrument(
        level = Level::DEBUG,
        fields(actor = TRACING_ACTOR, layer = TRACING_FIELD_LAYER_PROTOCOL, ceremony = TRACING_FIELD_CEREMONY_SETUP),
    )]
    pub fn initialize_with_identity(
        ...
        rpc_client_url: String,
        rpc_client_auth: BitcoinCoreAuth,
        wt_privkey: PrivateKey,
        wt_tor_secret_key: TorSecretKey,
    ) -> Result<(), error::LoadError> {
        function_start_log!();
    ...
    pub fn consume_setup_niso_wt_message_1(
        ...
        let (Some(wt_privkey), Some(wt_id)) = (&self.wt_privkey, &self.wt_id) else {
            let err = error::ConsumeSetupNisoWtMessage1Error::InternalInvariant;
            error_log!(
                err,
                "WT setup state is missing key material required by this entrypoint."
            );
            return Err(err);
        };
...
#[cfg(test)]
mod tests {
    use crate::WtCreateParams;
    use cryptography::PublicKey;
    use protocol::messages::{Parcel, setup::from_niso::to_wt::SetupNisoWtMessage1};

    use super::*;

    #[test]
    fn consume_setup_niso_wt_message_1_returns_internal_invariant_error_when_state_data_is_missing()
    {
        let mut wt = Wt::create_with_params(WtCreateParams::new(0, 0, 0, 0, 0, 0, 0, 0, 0));
        wt.state = State::Setup_AfterLoad_SetupReadyToRegisterService;

        let result = wt.consume_setup_niso_wt_message_1(
            Parcel::<PublicKey, SetupNisoWtMessage1>::new(Vec::new()),
        );

        assert!(matches!(
            result,
            Err(error::ConsumeSetupNisoWtMessage1Error::InternalInvariant)
        ));
    }
}
