...
impl Sar {
    ...
    /// Initialize SAR using pre-generated identity material so supervisors can
    /// keep peer bootstrap identities aligned with the running process.
    #[instrument(
        level = Level::DEBUG,
        fields(actor = TRACING_ACTOR, layer = TRACING_FIELD_LAYER_PROTOCOL, ceremony = TRACING_FIELD_CEREMONY_SETUP),
    )]
    pub fn initialize_with_identity(
        &mut self,
        sar_privkey: PrivateKey,
        sar_tor_secret_key: TorSecretKey,
