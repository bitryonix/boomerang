...
mod tests {
    ...
    #[test]
    fn create_starts_with_blank_setup_state_and_no_loaded_material() {
        let iso = Iso::create();

        assert_eq!(iso.state, State::Setup_AfterCreation_BlankSlate);
        assert!(iso.network.is_none());
        assert!(iso.mnemonic.is_none());
        assert!(iso.boomlet_identity_pubkey.is_none());
        assert!(iso.withdrawal_key_agg_context.is_none());
    }
}
