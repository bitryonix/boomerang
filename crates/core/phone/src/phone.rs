...
mod tests {
    ...
    #[test]
    fn create_starts_with_blank_setup_state_and_no_doxing_material() {
        let phone = Phone::create();

        assert_eq!(phone.state, State::Setup_AfterCreation_BlankSlate);
        assert!(phone.doxing_key.is_none());
        assert!(phone.static_doxing_data.is_none());
        assert!(phone.dynamic_doxing_data.is_none());
    }
}
